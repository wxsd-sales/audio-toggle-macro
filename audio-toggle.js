...
const groupIconIds = {
  [groupModeOn]: `${panelId}Green`,
  [groupModeOff]: `${panelId}Red`,
  [groupModeMixed]: `${panelId}Orange`,
};
let groupModes = [];
...
async function init() {
  ...
  if (!allowUserCancelDefaults) return;

  xapi.Event.UserInterface.Message.Prompt.Response.on(
    ({ FeedbackId, OptionId }) => {
      if (FeedbackId != panelId) return;
...
function processCalls(value) {
  ...
  xapi.Command.UserInterface.Message.Prompt.Display({
    Duration: applyDefaultDelayMinutes * 60,
    FeedbackId: panelId,
    "Option.1": "Cancel Audio Reset",
    "Option.2": "Reset Audio Now",
    Text: `Audio settings will reset to defaults in ${applyDefaultDelayMinutes} minutes.`,
...
function createPanel(name, active, order) {
  const orderXml =
    typeof order == "undefined" ? "" : `<Order>${escapeXml(order)}</Order>`;
...
async function saveButtons() {
  const panels = groups.map(({ name }, i) => createPanel(name, groupModes[i]));
...
async function toggleButton(index) {
  ...
  const newState = currentIcon != groupIconIds[groupModeOn];
...
async function getGroupMode(group) {
  ...
  for (let i = 0; i < microphoneIds.length; i++) {
    ...
    const mode = await getXapiValue(`microphone ${microphoneId} mode`, () =>
  ...
  for (let i = 0; i < usbMicrophones.length; i++) {
    ...
    const mode = await getXapiValue(
      ...
      () => xapi.Config.Audio.Input.USBInterface[usbMicrophone].Mode.get(),
    );
...
async function setUsbMicrophonesMode(group, mode) {
  ...
  for (let i = 0; i < usbMicrophones.length; i++) {
    ...
    await setXapiValue(`USB microphone ${usbMicrophone} mode`, () =>
      xapi.Config.Audio.Input.USBInterface[usbMicrophone].Mode.set(mode),
    );
  }
}
